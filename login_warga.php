    ...
    if ($user->login()) {
        ...
        if ($row['level'] === 'admin') {
            ...
            try {
                ...
                $insert->execute([$user->id, $user->nik, $user->username, $user->level, 1, $ip, $ua]);
            } catch (Exception $e) {
        ...
        try {
            ...
            $ip = isset($_SERVER['REMOTE_ADDR']) ? $_SERVER['REMOTE_ADDR'] : null;
            $ua = isset($_SERVER['HTTP_USER_AGENT']) ? $_SERVER['HTTP_USER_AGENT'] : null;
            // user not found or wrong password -> user_id null
            $insert->execute([null, $_POST['nik'], null, 'warga', 0, $ip, $ua]);
        } catch (Exception $e) {
            error_log('Login history insert failed: ' . $e->getMessage());
        }
...
<head>
    ...
    <style>
        ...
        .login-wrapper { min-height:100vh; display:flex; flex-direction:column; background:transparent; }
        .login-header { background: linear-gradient(135deg, var(--primary) 0%, var(--secondary) 100%); color:var(--white); padding:2rem 0; text-align:center; box-shadow:0 6px 24px rgba(2,8,30,0.6); }
        .login-logo{ font-size:2rem; font-weight:700; }
        .login-subtitle{ color: rgba(255,255,255,0.9); }
        ...
        .login-form{ background: linear-gradient(180deg, rgba(255,255,255,0.03), rgba(255,255,255,0.02)); border:1px solid rgba(255,255,255,0.04); padding:2.25rem; border-radius:12px; box-shadow:0 10px 40px rgba(2,10,30,0.6); color:var(--app-text); }
        ...
        .form-control{ width:100%; padding:.75rem 1rem; border-radius:8px; background:rgba(255,255,255,0.02); border:1px solid rgba(255,255,255,0.04); color:var(--app-text); }
        ...
        .login-footer p{ color:var(--app-muted); }
        .login-footer a{ color:var(--secondary); font-weight:700; text-decoration:none; }

        .login-links a{ color:var(--secondary); }
        .page-footer{ background:transparent; color: rgba(255,255,255,0.6); text-align:center; padding:1rem 0; }
                ...
                <form method="POST" action="" class="login-form">
                    ...
                    <div class="login-footer">
                        ...
                        <div class="login-links">
                            <a href="login_admin.php">⚙️ Login Admin</a>
                        </div>
                    </div>
                </form>
            </div>
