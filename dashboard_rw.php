...
// Check if logged in as RW
if (!isLoggedIn() || $_SESSION['level'] !== 'rw') {
    header("Location: login_rw.php");
    exit();
}
...
$query = "SELECT 
    COUNT(*) as total,
    SUM(CASE WHEN status = 'approved_rt' THEN 1 ELSE 0 END) as waiting_approval,
    SUM(CASE WHEN status = 'approved_rw' THEN 1 ELSE 0 END) as approved,
    SUM(CASE WHEN status = 'rejected_rw' THEN 1 ELSE 0 END) as rejected
...
// Get letters waiting for RW approval (those already approved by RT)
$query = "SELECT s.*, u.nama as nama_warga, j.nama as jenis_surat 
    FROM surat s
    JOIN users u ON s.user_id = u.id
    JOIN jenis_surat j ON s.jenis_id = j.id
    WHERE s.status = 'approved_rt' 
    ORDER BY s.created_at DESC
    LIMIT 10";

$stmt = $db->prepare($query);
$stmt->execute();
$waiting_letters = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Get approved by RW letters
$query = "SELECT s.*, u.nama as nama_warga, j.nama as jenis_surat 
    FROM surat s
    JOIN users u ON s.user_id = u.id
    JOIN jenis_surat j ON s.jenis_id = j.id
    WHERE s.status = 'approved_rw'
    ORDER BY s.created_at DESC
    LIMIT 10";
...
<head>
    ...
    <title>Dashboard RW - Sistem Surat RT/RW</title>
...
<body>
    <nav class="navbar">
        <div class="navbar-left">
            <div class="navbar-title">🏗️ Dashboard RW</div>
            <span class="navbar-role">KETUA RUKUN WARGA</span>
        </div>
    ...
    <div class="container">
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card waiting">
                <div class="stat-label">⏳ Menunggu Persetujuan RW</div>
                <div class="stat-value"><?php echo $stats['waiting_approval'] ?? 0; ?></div>
            </div>
        ...
        <!-- Waiting Letters -->
        <div class="section">
            <h2 class="section-title">📋 Surat Menunggu Persetujuan RW</h2>
            
            <?php if (!empty($waiting_letters)): ?>
        ...
        <!-- Approved Letters -->
        <div class="section">
            <h2 class="section-title">✅ Surat Sudah Disetujui RW</h2>
    ...
    <script>
        ...
        function approveLetter(id) {
            if (confirm('Setujui surat ini?')) {
                window.location.href = `update_letter_status.php?id=${id}&status=approved_rw`;
            }
        }
        ...
        function rejectLetter(id) {
            ...
            if (reason) {
                window.location.href = `update_letter_status.php?id=${id}&status=rejected_rw&reason=${encodeURIComponent(reason)}`;
            }
        }
    </script>
</body>
